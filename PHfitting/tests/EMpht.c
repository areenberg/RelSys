...
void input_density()
{
  ...
  int choice = 6; //<<---- control input density type
  int choice1 = 1; //<<---- choose if PH input should be from file
  double truncpoint = 100.0; //<<---- truncation point
  double dt = truncpoint/500.0; //<<---- size of interval dt
  //int stepchoice = 1; //<<---- default choice for step length in Runge-Kutta method
...
{
  int input = 2; //<<---- control sample or density input
		     ...
		     int **Tlegal)
{
  ...
  int structure = 4; //<<--- structure of output PH distribution
...
int main() 
{
  ...
  int p = 3; //<<<--- number of phases to fit
  int NoOfEMsteps = 50; //<<<--- number of iterations
