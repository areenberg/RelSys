...
PhaseFitter::PhaseFitter(const PhaseFitter& orig) {
}
...
double *PhaseFitter::v_alloc(int n)
{
  double *v;
...
int *PhaseFitter::int_v_alloc(int n)
{
  int *v;
...
double **PhaseFitter::m_alloc(int n, int k)
{
  int i;
  double **mat;
...
int **PhaseFitter::int_m_alloc(int n, int k)
{
  int i, **mat;
...
double ***PhaseFitter::m3_alloc(int n, int k, int v)
{
  int i,j;
  double ***mat;
  mat=(double ***) calloc(n,sizeof(double **));
...
void PhaseFitter::init_vector(double *vector, int NoOfElements)
{
  int i;
...
void PhaseFitter::init_integervector(int *vector, int dim)
{
  int i;
...
void PhaseFitter::init_matrix(double **matrix, int dim1, int dim2)
{
  int i, j;
...
void PhaseFitter::init_integermatrix(int **matrix, int dim1, int dim2)
{
  int i, j;
...
void PhaseFitter::init_3dimmatrix(double ***matrix, int dim1, int dim2, int dim3)
{
  int i, j, k;
...
void PhaseFitter::free_matrix(double **matrix, int dim1)
{
  int i;
...
void PhaseFitter::free_integermatrix(int **matrix, int dim1)
{
  int i;
...
void PhaseFitter::free_3dimmatrix(double ***matrix, int dim1, int dim2)
{
  int i;
...
void PhaseFitter::a_rungekutta(int p, double *avector, double **ka, double dt, double h, 
		  double **T)
{
  int i,j;
  double eps, h2, sum;
...
void PhaseFitter::rungekutta(int p, double *avector, double *gvector, double *bvector, 
...
void PhaseFitter::ExportToEMPHTmain_Phasetype(int dim, double h, double dt,double truncpoint,
				 double *pie, double **q, double *exit)
{  
  int i,j;
...
void PhaseFitter::ExportToMatlab(int k, double *parameter, double truncpoint)
{
  double t, deltat;
...
void PhaseFitter::show_pi_T(int p, double *pi, double **T, double *t)
{
  
  int i, j;  
    
  init_dist.resize(p,0);
  exit_rate_vector.resize(p,0);
  ph_generator.resize(p);
    
  for (i=0; i < p; i++) {
    ph_generator[i].resize(p,0);  
    ...
    for (j=0; j < p; j++){
        ...
        ph_generator[i][j] = T[i][j];
        //printf("%lf ", T[i][j]);
                
        //exit rate vector
        exit_rate_vector[i] = t[i];
        //printf("   %lf",t[i]);
...
double PhaseFitter::set_steplength(int p, double **T)
{    
  int i;
  double h;
...
//  printf("Select (1-4): ");
//  scanf("%d", &choice);
  choice = 3; //always unweighted samples from file
  i=1;     
  switch(choice) {
  case 1:
//    Weight=1;
...
//    fclose(utfil);
  ...
  case 3:
    ...
    //get input/sample size from
    //the sample vector  
    NoOfInput[1] = sample_vector.size();  
...
int PhaseFitter::sort_interval(int NoOfPairs)
{
  double temp;
  int i, j, v, newsize;
...
void PhaseFitter::Export_Sample()
{
  int i;
  FILE *utfil2;
  double CumWeight;
...
void PhaseFitter::Export_CensoredSample(int NoOfInput[3])
{
  int i, j, risk;
  double kaplan;
  FILE *utfil2;
...
int PhaseFitter::AskForInput(int NoOfInput[3])
{
  int input = sampleOrDensity; //control sample or density input
  int sampletype; //if sample, control censored or non-censored
...
*/  
    ...
    case 1:
      ...
      assign_vectors(NoOfInput[1]); //stores sample values and weights in arrays obs and weight
...
void PhaseFitter::randomphase(int p, double *pi, double **T, double *exitvector, 
...
void PhaseFitter::selectstructure(int p, double *pi, double **T, double *t, int *pilegal,
		     int **Tlegal)
{
  int i, j;
  FILE *infil;
...
void PhaseFitter::EMstep(int p, double h, double *pi, double **T, double *t, double 
...
void PhaseFitter::compute_loglikelihood(double h, int p, double *pi, double **T, double *t, 
...
void PhaseFitter::SavePhases(int p, double *pi, double **T)
{ 
  int i,j;
  FILE *utfil;
...
void PhaseFitter::EMiterate(int NoOfEMsteps, int p, double *pi, double **T, double *t,
...
void PhaseFitter::run(int NoOfEMsteps, int intSeed, int inDType, double trP, double dt) {
    ...
    /*
     ...
     intSeed: Seed used to initialize the EM algorithm.
     inDType: If density, input density type: 1=Rectangle,2=Normal,3=Lognormal,4=Weibull,5=Inverse Gauss,6=Phasetype,7=User specified.
     trP: If density, time truncation point.
     dt: If density, size of time-intervals. 
     */
...
void PhaseFitter::setGeneralPH(int p=2){
    //set output PH distribution to general phase-type
    //p = Number of phases in output PH distribution.
    phases = p; 
    PHDistribution = 1;
}

void PhaseFitter::setHyberExponential(int p=2){
...
void PhaseFitter::setSumOfExponentials(int p=2){
    //set output PH distribution to sum of exponentials.
    //p = Number of phases in output PH distribution.
    phases = p; 
    PHDistribution = 3;
}

void PhaseFitter::setCoxian(int p=2){
    //set output PH distribution to Coxian.
    //p = Number of phases in output PH distribution.
    phases = p; 
    PHDistribution = 4;
}

void PhaseFitter::setCoxianGeneral(int p=2){
    //set output PH distribution to Coxian general.
    //p = Number of phases in output PH distribution.
    phases = p; 
    PHDistribution = 5;
}

void PhaseFitter::setInputSample(vector<double> &samples){
    //fits the phase-type distribution to samples
    
    sample_vector.resize(samples.size(),0);
    
    //copy and store all sample values
    for (int i=0; i<samples.size(); i++){
        sample_vector[i] = samples[i];
    }
    
    //weights are all set to 1 (for now)
    weight_vector.resize(samples.size(),1);
    
    //tell the program that the input uses sampled observations
    sampleOrDensity = 1;
